...
def show_insights():
    ...
    try:
        ...
        if all(col in df.columns for col in ['ticker_symbol', 'share_count']):
            ...
            tickers_data = yf.download(tickers=tickers, period='1d', interval='1m', auto_adjust=True)['Close'].iloc[-1].ffill().bfill()
            ...
            **Formatting Rules**:  
            ...
            - Do NOT mention that you "don't have access" or "data isn't provided" for certain kinds of data. Simply don't reference areas that require more data than provided.
