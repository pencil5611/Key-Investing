...
def show_port_manager():
    tab1, tab2, tab3 = st.tabs(["Portfolio Management", "Transaction History", 'Portfolio Risk Analysis'])
        ...
        try:
            ...
            if all(col in df.columns for col in ['ticker_symbol', 'share_count']):
                ...
                if not df.empty:
                    ...
                    portfolio_prices = yf.download(tickers, start=start_date, auto_adjust=True, progress=False)['Close']
    ...
    def portfolio_risk():
        ...
        try:
            ...
            latest_prices = yf.download(tickers=tickers, period='1d', interval='1d', auto_adjust=True, progress=False)['Close'].iloc[-1]
